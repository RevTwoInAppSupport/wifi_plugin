...
class Wifi {
  ...
  static Future<List<WifiResult>> list(String key) async {
    ...
    for (int i = 0; i < results.length; i++) {
      resultList.add(WifiResult(results[i]['ssid'], results[i]['level'],results[i]['capabilities']));
    }
    return resultList;
  }
  ...
  static Future<void> remove(String ssid) async {
    final Map<String, dynamic> params = {
      'ssid': ssid,
    };
    await _channel.invokeMethod('remove', params);
    return;
  }
...
class WifiResult {
  ...
  WifiResult(this.ssid, this.level,this.capabilities);
}
